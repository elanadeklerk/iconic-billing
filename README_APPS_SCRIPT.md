...
function doPost(e) {
  try {
    ...
    // Format the entire tariff/icd10/modifier columns as plain text once per call.
    // This is more reliable than per-row formatting — prevents Sheets from
    // auto-converting codes like "0190,0191" into a large number.
    sheet.getRange('H:J').setNumberFormat('@STRING@');

    var rowsToAdd = Array.isArray(data.rows) ? data.rows : [data];

    rowsToAdd.forEach(function(row) {
      var nextRow = sheet.getLastRow() + 1;
      // Write non-code columns together
      ...
      // Write code columns individually — most reliable way to force text storage
      sheet.getRange(nextRow, 8).setValue(String(row.tariff   || ''));
      sheet.getRange(nextRow, 9).setValue(String(row.icd10    || ''));
      sheet.getRange(nextRow, 10).setValue(String(row.modifier || ''));
      sheet.getRange(nextRow, 11).setValue(row.notes || '');
    });
